...
class AgentTools:
    ...
    def find_files(pattern: str, directory: str = ".") -> str:
        try:
            matches = _glob.glob(f"{directory}/**/{pattern}", recursive=True)
            if not matches:
    ...
    def list_processes() -> str:
        try:
            result = subprocess.run(
                ["ps", "aux", "--sort=-%cpu"] if platform.system() != "Windows"
                else ["tasklist"],
    ...
    def schemas(cls) -> list[dict]:
        """Return JSON tool schemas for all registered tools."""
...
# --------------------------------------------------------------------------- #
# JSON schemas exposed to Claude
# --------------------------------------------------------------------------- #
...
_TOOL_SCHEMAS: list[dict] = [
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {"path": {"type": "string", "description": "Save path (default /tmp/screenshot.png)"}},
        },
    },
    {
